...
public class VeiculoDAO {
    ...
    public ResultSet consultarInstrutorVeiculo(VeiculoDTO veiculoDTO, int opcao) {
        try {
            ...
            switch (opcao) {
                case 1:
                    comando = "SELECT * FROM veiculoinstrutor "
                    		+ "vi inner join instrutor i "
                    		+ "on i.id_instrutor = vi.id_instrutor "
                    		+ "inner join veiculo ve "
                    		+ "on ve.placa = vi.placa";
                    
                    
                    break;
                case 2:
                    comando = "SELECT v.* FROM veiculoinstrutor v WHERE "
                            + "v.placa like '" + veiculoDTO.getPlaca() + "%'";
                    break;
                case 3:
                    comando = "SELECT * FROM veiculoinstrutor WHERE "
